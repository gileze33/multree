...
- `repos.<name>.commands.<target>.<action>` — repo-scoped runnable commands, dispatched as `multree <action> <group> <target>`. Each key under `commands` is a target (typically a package in a monorepo); each key under a target is an action verb you name yourself, so adding `run`/`build`/`test` costs a manifest key and no code. The reserved `cwd` key is the subdirectory every action runs in — omit it to run at the worktree root. An action's value is a shell string, an argv array, or `{ command, cwd }` to override the target's `cwd`; `{cwd}` is substituted, as for tools, and the value executes the same way — a string under non-interactive `bash`, an argv array directly. Targets are addressed flat, so a target name must be unique across every repo in a group.
...
Both `tools` and `commands` execute the same way, and not in your interactive shell. A string value runs under non-interactive `bash`, whatever `$SHELL` is set to; an argv array is exec'd directly, with no shell at all. Neither sources `.bashrc`, `.zshrc` or any other rc file, so shell aliases and functions are unreachable from a `tools` or `commands` entry — write the expansion out, or set the environment inline (`env FOO=bar mycmd`), instead of relying on an alias. Exported environment variables do carry through, since the command inherits multree's own environment. `shell` is the exception: it starts your `$SHELL` interactively, so aliases are available there.
