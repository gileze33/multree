...
- **prime_artifacts**: declared at the manifest level, per repo, or both. A repo's effective list is its own entries followed by the manifest's, with a later duplicate of the same target dropped — so a repo overrides an inherited target simply by naming it. `resolvePrimeArtifacts()` in `config.ts` is the only sanctioned read; both read sites (`phases.ts`, `create --plan`) go through it. Each entry has either `path` or `find` (basename to locate recursively in the source repo — directories only), and a `strategy`: `copy`, `reflink` (`cp -c` / APFS clone, so trees like `node_modules` are reconciled rather than cold-installed), or `symlink` (links the worktree path at the main checkout's, so writes resolve there). Destination occupancy is `lstat`-based for every strategy, so an existing link — including a dangling one — is skipped rather than written over. Structural defects and the one provable collision (a `path`-addressed `symlink` covering a `consumes`/`exposes` file) are rejected in `validate()` at load; commands that only inspect or tear down a group pass `tolerateInvalidPrimeArtifacts` so a bad entry can't lock them out.
